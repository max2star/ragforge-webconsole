...
const OllamaEditModal = ({
  visible,
  hideModal,
  onOk,
  loading,
  llmFactory,
}: IModalProps<IAddLlmRequestBody> & { llmFactory: any }) => {
  ...
  const url =
    llmFactoryToUrlMap[llmFactory.llmFactory as LlmFactory] ||
    'https://github.com/infiniflow/ragflow/blob/main/docs/guides/models/deploy_local_llm.mdx';
  const optionsMap = {
    [LLMFactory.HuggingFace]: [
  ...
  useEffect(() => {
    const { api_base, api_key, name, type, used_token } = llmFactory;
    form.setFieldsValue({
      model_type: type,
      llm_name: name,
      api_base,
      api_key,
      max_tokens: used_token,
      vision: false,
    });
  }, [JSON.stringify(llmFactory)]);
  return (
    <Modal
      title={t('editLlmTitle', { name: llmFactory.llmFactory })}
      open={visible}
      onOk={handleOk}
      onCancel={hideModal}
        ...
        return (
          <Flex justify={'space-between'}>
            <a href={url} target="_blank" rel="noreferrer">
              {t('ollamaLink', { name: llmFactory.llmFactory })}
            </a>
      ...
      >
        ...
        >
          <Select placeholder={t('modelTypeMessage')}>
            {getOptions(llmFactory.llmFactory).map((option) => (
        ...
        <Form.Item<FieldType>
          label={t(
            llmFactory.llmFactory === 'Xinference' ? 'modelUid' : 'modelName',
          )}
          name="llm_name"
...
export default OllamaEditModal;
